...
class ControllerPaymentnovalnetelvatpci extends Controller {
	...
	protected function index() {
	    ...
	    $this->language->load('payment/novalnet_elv_at_pci');
	    $this->data['text_austriapci'] = $this->language->get('text_austriapci');
		...
		$this->data['testmode_desc'] = html_entity_decode($this->config->get('novalnet_elv_at_pci_testmode'), ENT_QUOTES, 'UTF-8');
	    $this->data['text_heading'] = $this->language->get('text_heading');
	    $_SESSION['novalnet_elv_at_pci_order_info'] = 	 $order_info = $this->model_checkout_order->getOrder($this->session->data['order_id']);
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/novalnet_elv_at_pci.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/novalnet_elv_at_pci.tpl';
		} else {
			$this->template = 'default/template/payment/novalnet_elv_at_pci.tpl';
		}	
		...
		if(isset($_POST['status'])) {
		    if (!$this->checkHash($_POST)&& $_POST['status']==100){
			   ...
			   if($_POST['status']==100){
				  ...
				  $novalnet_elv_at_pci_comments ='';
		          $testmode  = html_entity_decode($this->config->get('novalnet_elv_at_pci_testmode'), ENT_QUOTES, 'UTF-8');
		          $test_order_status = (((isset($_POST['test_mode']) && $_POST['test_mode'] == 1) || (isset($testmode) && $testmode == 1)) ? 1 : 0 );
				   if(isset($test_order_status)){
					$novalnet_elv_at_pci_comments .= $this->language->get('text_elv_at_pci_testorder')."<br />";
				  }
                  $novalnet_elv_at_pci_comments .= $this->language->get('text_elv_at_pci_transactionid').$_POST['tid'];	
	              ...
	              if(isset($_POST['tid']) && $_POST['tid'] != '' )
		          {
		            ...
		            $urlparam = array(
		                  'vendor'          =>  $this->elv_at_pci_vendor_id,
		                  'product'         =>  $this->product_id,
		                  'tariff'          =>  $this->tariff_id,  
		                  'key'             =>  $this->key,  
		                  'auth_code'       =>  $this->elv_at_pci_authcode,
		                  'status'          =>  '100',
		                  'tid'             =>  $_SESSION['nn_tid'],
		                  'order_no'        =>  $order_id,
		                  '&vwz2'           =>  $order_id,
                  ...
                  $this->model_checkout_order->confirm($_POST['order_no'], $this->config->get('novalnet_elv_at_pci_order_status_id'),$novalnet_elv_at_pci_comments,'true');
				  ...
				  if(isset($_POST['status_desc']))  {
				   ...
				   $error_status_desc = $this->language->get('text_elv_at_status');
				  }
	...
	public function confirm() {
		...
		$order_info = $_SESSION['novalnet_elv_at_pci_order_info'];
		...
		$status_url = $this->url->link('payment/novalnet_elv_at_pci/callback','','SSL');
		$urlparam = array(    'vendor_id'           => $this->elv_at_pci_vendor_id,
    ...
    public function params(){
		...
		$this->elv_at_pci_product_id = html_entity_decode(trim($this->config->get('novalnet_elv_at_pci_productid')),ENT_QUOTES, 'UTF-8');  
	    $this->elv_at_pci_tariff_id = html_entity_decode(trim($this->config->get('novalnet_elv_at_pci_tariffid')),ENT_QUOTES, 'UTF-8');
	    $this->elv_at_pci_manual_check_limit = html_entity_decode(trim($this->config->get('novalnet_elv_at_pci_manualcheck')),ENT_QUOTES, 'UTF-8');
	    $this->elv_at_pci_product_id2 = html_entity_decode(trim($this->config->get('novalnet_elv_at_pci_productid2')),ENT_QUOTES, 'UTF-8');
	    $this->elv_at_pci_tariff_id2 = html_entity_decode(trim($this->config->get('novalnet_elv_at_pci_tariffid2')),ENT_QUOTES, 'UTF-8');
	    $this->elv_at_pci_authcode  = html_entity_decode(trim($this->config->get('novalnet_elv_at_pci_auth_code')), ENT_QUOTES, 'UTF-8');
	    $this->elv_at_pci_vendor_id = html_entity_decode(trim($this->config->get('novalnet_elv_at_pci_merchantid')), ENT_QUOTES,'UTF-8');
	    $this->elv_at_pci_test_mode = html_entity_decode(trim($this->config->get('novalnet_elv_at_pci_testmode')), ENT_QUOTES, 'UTF-8');
        ...
        $this->product_id   = $this->elv_at_pci_product_id;
        $this->tariff_id    = $this->elv_at_pci_tariff_id;
        $manual_check_limit = $this->elv_at_pci_manual_check_limit;
        ...
        if($manual_check_limit && $this->amount>=$manual_check_limit)
           ...
           $this->product_id = $this->elv_at_pci_product_id2;
           $this->tariff_id  = $this->elv_at_pci_tariff_id2;
          }
	...
	public function sendForm($url, $aParams) {
		...
		$this->language->load('payment/novalnet_elv_at_pci');
				...
				$frmEnd  = $this->language->get('text_elv_at_pci_automaticredirection').'<br> <input type="submit" name="enter" value='.$this->language->get('text_elv_at_pci_redirection').' /></form>';
    ...
    try {
      ...
      $data = $crc."|".$data;
      $data = bin2hex($data.html_entity_decode(trim($this->config->get('novalnet_elv_at_pci_password')), ENT_QUOTES, 'UTF-8'));
    ...
    function hash($h)#$h contains encoded data
      ...
      global $amount_zh;
       ...
       return md5($h['authcode'].$h['product_id'].$h['tariff'].$h['amount'].$h['test_mode'].$h['uniqid'].strrev(html_entity_decode(trim($this->config->get('novalnet_elv_at_pci_password')), ENT_QUOTES, 'UTF-8')));
      }
    ...
    function decode($data){
       ...
       if (!function_exists('base64_decode') or !function_exists('pack') or !function_exists('crc32')){return'Error: func n/a';}
        ...
        $data = substr($data, 0, stripos($data, html_entity_decode(trim($this->config->get('novalnet_elv_at_pci_password')), ENT_QUOTES, 'UTF-8')));
        $pos = strpos($data, "|");
          if ($pos === false){
