...
class ControllerPaymentnovalnetelvdepci extends Controller {
	...
	protected function index() {
	    ...
	    $this->language->load('payment/novalnet_elv_de_pci');
	    $this->data['text_germanpci'] = $this->language->get('text_germanpci');
		...
		$this->data['testmode_desc'] = html_entity_decode($this->config->get('novalnet_elv_de_pci_test'), ENT_QUOTES, 'UTF-8');
	    $this->data['text_novalnet_redirection'] = $this->language->get('text_novalnet_redirection');
	    $_SESSION['novalnet_elv_de_pci_order_info'] = 	 $order_info = $this->model_checkout_order->getOrder($this->session->data['order_id']);
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/novalnet_elv_de_pci.tpl')) {
			$this->template = $this->config->get('config_template') . '/template/payment/novalnet_elv_de_pci.tpl';
		} else {
			$this->template = 'default/template/payment/novalnet_elv_de_pci.tpl';
		}	
		...
		$this->language->load('payment/novalnet_elv_de_pci');
		 if(isset($_POST['status'])){
			...
			if($_POST['status']==100){
			  ...
			  $novalnet_elv_de_pci_comments ='';
			  $testmode  = html_entity_decode($this->config->get('novalnet_elv_de_pci_test'), ENT_QUOTES, 'UTF-8');
		          $test_order_status = (((isset($_POST['test_mode']) && $_POST['test_mode'] == 1) || (isset($testmode) && $testmode == 1)) ? 1 : 0 );
				   if(isset($test_order_status))
		            {
					 $novalnet_elv_de_pci_comments .=$this->language->get('text_elv_de_pci_testorder')."<br />";
					}
				$novalnet_elv_de_pci_comments .= $this->language->get('text_elv_de_pci_transactionid').$_POST['tid'];
	          ...
	          if(isset($_POST['tid']) && $_POST['tid'] != '' )
		      {
		            ...
		            $urlparam = array(
		                  'vendor'          =>  $this->elv_de_pci_vendor_id,
		                  'product'         =>  $this->product_id,
		                  'tariff'          =>  $this->tariff_id,
		                  'key'             =>  $this->key,  
		                  'auth_code'       =>  $this->elv_de_pci_authcode,
		                  'status'          =>  '100',
		                  'tid'             =>  $_SESSION['nn_tid'],
		                  'order_no'        =>  $order_id,
		                  '&vwz2'           =>  $order_id,
              ...
              $this->model_checkout_order->confirm($_POST['order_no'], $this->config->get('novalnet_elv_de_pci_order_status_id'),$novalnet_elv_de_pci_comments,'true');
			  $this->redirect($this->shop_url .'index.php/?route=checkout/success');	  
			    ...
			    if(isset($_POST['status_desc']))  {
				 ...
				 $error_status_desc = $this->language->get('text_elv_de_pci_status');
			     }	  
	...
	public function confirm() {
		...
		$order_info = $_SESSION['novalnet_elv_de_pci_order_info'];
		...
		if($order_info['customer_id'] == 0 ||  $order_info['customer_id'] == '') {
			...
			$status_url = $this->url->link('payment/novalnet_elv_de_pci/callback','','SSL');
			$urlparam = array(    'vendor_id'                                       => $this->elv_de_pci_vendor_id,
    ...
    public function params(){
		...
		$this->elv_de_pci_product_id = html_entity_decode(trim($this->config->get('novalnet_elv_de_pci_productid')),ENT_QUOTES, 'UTF-8');  
	    $this->elv_de_pci_tariff_id = html_entity_decode(trim($this->config->get('novalnet_elv_de_pci_tariffid')),ENT_QUOTES, 'UTF-8');
	    $this->elv_de_pci_manual_check_limit = html_entity_decode(trim($this->config->get('novalnet_elv_de_pci_manualamount')),ENT_QUOTES, 'UTF-8');
	    $this->elv_de_pci_product_id2 = html_entity_decode(trim($this->config->get('novalnet_elv_de_pci_productid2')),ENT_QUOTES, 'UTF-8');
	    $this->elv_de_pci_tariff_id2 = html_entity_decode(trim($this->config->get('novalnet_elv_de_pci_tariffid2')),ENT_QUOTES, 'UTF-8');
	    $this->elv_de_pci_authcode  = html_entity_decode(trim($this->config->get('novalnet_elv_de_pci_authorisation')), ENT_QUOTES, 'UTF-8');
	    $this->elv_de_pci_vendor_id = html_entity_decode(trim($this->config->get('novalnet_elv_de_pci_merchantid')), ENT_QUOTES,'UTF-8');
	    $this->elv_de_test_mode = html_entity_decode(trim($this->config->get('novalnet_elv_de_pci_test')), ENT_QUOTES, 'UTF-8');
		...
		$this->product_id  = $this->elv_de_pci_product_id;
		$this->tariff_id   = $this->elv_de_pci_tariff_id;
		$manual_check_limit = $this->elv_de_pci_manual_check_limit;
		$manual_check_limit = str_replace(',', '', $manual_check_limit);
		$manual_check_limit = str_replace('.', '', $manual_check_limit);
      
		if($manual_check_limit && $this->amount>=$manual_check_limit){
		  $this->product_id = $this->elv_de_pci_product_id2;
		  $this->tariff_id = $this->elv_de_pci_tariff_id2;
		}
	...
	public function sendForm($url, $aParams) {
		if( $aParams ) {
		   $this->language->load('payment/novalnet_elv_de_pci');
			...
			$frmEnd  =$this->language->get('text_elv_de_pci_automaticredirection').'<br> <input type="submit" name="enter" value='.$this->language->get('text_elv_de_pci_redirection').' /></form>';
    ...
    function encode($data){
		...
		try {
		  ...
		  $data = $crc."|".$data;
		  $data = bin2hex($data.html_entity_decode(trim($this->config->get('novalnet_elv_de_pci_password')), ENT_QUOTES, 'UTF-8'));
    ...
    return md5($h['authcode'].$h['product_id'].$h['tariff'].$h['amount'].$h['test_mode'].$h['uniqid'].strrev(html_entity_decode(trim($this->config->get('novalnet_elv_de_pci_password')), ENT_QUOTES, 'UTF-8')));
   }
      ...
      $data = trim($data);
        if ($data == '') {return'Error: no data';}
         ...
         try {
           ...
           $data = substr($data, 0, stripos($data, html_entity_decode(trim($this->config->get('novalnet_elv_de_pci_password')), ENT_QUOTES, 'UTF-8')));
           $pos = strpos($data, "|");
             if ($pos === false){
