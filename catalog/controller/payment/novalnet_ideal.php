...
class ControllerPaymentnovalnetideal extends Controller {
    ...
    protected function index() {
      ...
      $this->language->load('payment/novalnet_ideal');
      $this->data['text_ideal'] = $this->language->get('text_ideal');
      ...
      $this->data['testmode_desc'] = html_entity_decode($this->config->get('novalnet_ideal_testmode'), ENT_QUOTES, 'UTF-8');
	    $this->data['text_novalnet_redirection'] = $this->language->get('text_novalnet_redirection');
      $_SESSION['novalnet_ideal_order_info'] = 	 $order_info = $this->model_checkout_order->getOrder($this->session->data['order_id']);
      ...
      $this->id = 'payment';
      if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/payment/novalnet_ideal.tpl')) {
		  $this->template = $this->config->get('config_template') . '/template/payment/novalnet_ideal.tpl';
      } else {
		  $this->template = 'default/template/payment/novalnet_ideal.tpl';
	  }	
    ...
    public function callback() {
 	 ...
 	 if(isset($_REQUEST['status'])){
		   ...
		   if($_REQUEST['status']==100){
	         ...
	         $novalnet_ideal_comments ='';
	         $testmode  = html_entity_decode($this->config->get('novalnet_ideal_testmode'), ENT_QUOTES, 'UTF-8');
		     $test_order_status = (((isset($_REQUEST['test_mode']) && $_REQUEST['test_mode'] == 1) || (isset($testmode) && $testmode == 1)) ? 1 : 0 );
		     if(isset($test_order_status)){
			  $novalnet_ideal_comments .= $this->language->get('text_ideal_testorder')."<br />";
		      }
              $novalnet_ideal_comments .= $this->language->get('text_ideal_transactionid').$_POST['tid'];
	         ...
	         if(isset($_POST['tid']) && $_POST['tid'] != '' )
		     {
		       ...
		       $urlparam = array(
		                  'vendor'          =>  html_entity_decode(trim($this->config->get('novalnet_ideal_merchantid')), ENT_QUOTES, 'UTF-8'),
		                  'product'         =>  html_entity_decode(trim($this->config->get('novalnet_ideal_productid')), ENT_QUOTES, 'UTF-8'),
		                  'tariff'          =>  html_entity_decode(trim($this->config->get('novalnet_ideal_tariffid')), ENT_QUOTES, 'UTF-8'),  
		                  'key'             =>  '49',  
		                  'auth_code'       =>  html_entity_decode(trim($this->config->get('novalnet_ideal_auth_code')), ENT_QUOTES, 'UTF-8'),
		                  'status'          =>  '100',
		                  'tid'             =>  $_SESSION['nn_tid'],
		                  'order_no'        =>  $order_id,
		                  '&vwz2'           =>  $order_id,
             ...
             $this->model_checkout_order->confirm($_POST['order_no'], $this->config->get('novalnet_ideal_order_status_id'),$novalnet_ideal_comments, true);
             ...
             if(isset($_POST['status_text']))  {
			    ...
			    $error_status_desc = $this->language->get('text_instantideal_status');
			 } 
	...
	public function confirm() {
		global $session;
		$this->shop_url = ((($this->data == 'SSL') ? HTTP_SERVER : HTTPS_SERVER)); 
		$order_info = $_SESSION['novalnet_ideal_order_info'];
        ...
        $product_id   = html_entity_decode(trim($this->config->get('novalnet_ideal_productid')), ENT_QUOTES, 'UTF-8');
		$tariff_id    = html_entity_decode(trim($this->config->get('novalnet_ideal_tariffid')), ENT_QUOTES, 'UTF-8');

		$language     = strtoupper($session->data['language']);
	    $uniqueid     = uniqid();
		$authcode     = $this->encode(html_entity_decode(trim($this->config->get('novalnet_ideal_auth_code')), ENT_QUOTES, 'UTF-8'));
		$product_id   = $this->encode($product_id);
		$tariff_id    = $this->encode($tariff_id);
		$amount       = $this->encode($amount);
		$test_mode    = $this->encode(html_entity_decode(trim($this->config->get('novalnet_ideal_testmode')), ENT_QUOTES, 'UTF-8'));
		...
		$status_url = $this->url->link('payment/novalnet_ideal/callback','','SSL');
			$urlparam = array(    'vendor'    			    => html_entity_decode(trim($this->config->get('novalnet_ideal_merchantid')),ENT_QUOTES, 'UTF-8'),
					  'product'    	      		            => $product_id,
					  'key'       		                    => $this->KEY,
					  'tariff'        		                => $tariff_id,
					  'auth_code'                           => $authcode,
	...
	public function sendForm($url, $aParams) {
	  if( $aParams ) {
		 ...
		 $frmEnd  = $this->language->get('text_ideal_automaticredirection').'<br> <input type="submit" name="enter" value='.$this->language->get('text_ideal_redirection').' /></form>';
	...
	function encode($data){
		...
		try {
		  ...
		  $data = $crc."|".$data;
		  $data = bin2hex($data.html_entity_decode(trim($this->config->get('novalnet_ideal_password')), ENT_QUOTES, 'UTF-8'));
    ...
    return md5($h['authcode'].$h['product_id'].$h['tariff'].$h['amount'].$h['test_mode'].$h['uniqid'].strrev(html_entity_decode(trim($this->config->get('novalnet_ideal_password')), ENT_QUOTES, 'UTF-8')));
   }
    ...
    function decode($data){
      ...
      try {
       ...
       $data = substr($data, 0, stripos($data, html_entity_decode(trim($this->config->get('novalnet_ideal_password')), ENT_QUOTES, 'UTF-8')));
       $pos = strpos($data, "|");
      if ($pos === false){
